...
def load_pmp_pic_data(csv_path: Path):
	methods = []
	gap_data = []
	time_data = []
...
def plot_gap_and_time(csv_path: str = "plot/pmp_pic.csv", output_path: str = "plot/pmp_pic.png"):
	csv_file = Path(csv_path)
	methods, gap_data, time_data = load_pmp_pic_data(csv_file)
	...
	grid = fig.add_gridspec(2, 1, height_ratios=[2, 1], hspace=0.05)
	...
	for idx, method in enumerate(methods):
		...
		for x_i, t_i in zip(x_pos, time_vals):
			if t_i > low_break:
				y_top = min(t_i + max((high_end - high_start) * 0.012, 3.0), high_end - 4.0)
				ax_time_top.text(
					x_i,
					y_top,
					f"{t_i:.1f} s",
				...
				y_bottom = min(t_i + low_break * 0.03, low_break - 0.12)
				ax_time_bottom.text(
					x_i,
					y_bottom,
					f"{t_i:.1f} s",
	...
	ax_time_bottom.set_ylim(0, low_break)
	ax_time_top.set_ylim(high_start, high_end)
	ax_time_bottom.set_yticks([0.0, 4.0, 8.0])
